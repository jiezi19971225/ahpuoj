    ...
    .content__main
      ...
      .main(ref="solutionContent")
        ...
        .main__section(v-if="solution && solution.contest_id == 0 && solution.result == 4 && $store.getters.userId == solution.user.id")
          ...
          p.mt10 当前状态
            el-tag(:type="solution.public == 0 ? 'danger':'success'",effect="dark") {{solution.public == 0 ? "不公开":"公开"}}
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (self.solution.result < 4) {
          ...
          self.timer = setInterval(async () => {
            ...
            const { data2 } = res2;
            self.seeable = data2.seeable;
            self.solution = data2.solution;
            if (self.solution.result >= 4) {
    ...
    async handleToggleSolutionStatus() {
      const self = this;
      try {
        const res = await toggleSolutionStatus(self.solution.id);
